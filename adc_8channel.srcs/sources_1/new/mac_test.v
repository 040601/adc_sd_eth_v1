  ...
  begin
    if (~rst_n)
      ...
      begin
        cnt_wr <= cnt_wr+1;
        cnt_rd <= cnt_rd+1;
        buffer[cnt_wr]<= adc_board1_adc1_b;
//        if(active)
//           tem_data <= buffer[cnt_rd];
      end
  end
always @(posedge gmii_tx_clk or negedge rst_n)
    begin
    ...
    if(active||num!=0)
      tem_data <= 12'h800-buffer[cnt_rd];
    else 
      tem_data <=0;
    end
//end code
...
always@(posedge gmii_rx_clk or negedge rst_n)
  begin
    if(rst_n == 1'b0)
       ...
       udp_send_data_length <= 200 ;//length to transmit
  end
...
localparam END2 = 2 ;
localparam TRANSITION = 3;

  reg [1:0] num_state; 
  reg [1:0] num_next_state;
   always@(posedge gmii_tx_clk or negedge rst_n)
    begin
      if(rst_n == 1'b0)
         num_state =IDLE;
     ...
     always@(*)
      begin
       ...
       begin
          if(state ==  WRITE_RAM)
            num_next_state <= COUNTER;
          else
           num_next_state <= IDLE2;
       end
       COUNTER:
       begin
         if(num == 100)
  ...
  always@(posedge gmii_tx_clk or negedge rst_n)
          begin
            if(rst_n == 1'b0)
              ...
              begin
       //         if(~active)//write over
                if(num_state == END2)//depend on the transmit side
                  begin
                    ram_wr_en <=1'b0;
                    write_end <= 1'b1;
                  end
                else//going on writing
                  begin
                    ram_wr_en <= 1'b1 ;
                    write_end <= 1'b0 ;
                    ram_wr_data <= tem_data[11:4];
                  end
              end
            else
              begin
         ...
         always@(posedge gmii_tx_clk or negedge rst_n)
           begin
             ...
             else if(num_state == COUNTER)
                num<= num+1;
             else num <= 0;
           end
  
 //move data from udp_data to ram_write_data
//always@(posedge gmii_tx_clk or negedge rst_n)
//  begin
//    if(rst_n == 1'b0)
//      begin
